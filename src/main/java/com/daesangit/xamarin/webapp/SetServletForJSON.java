...
public class SetServletForJSON extends HttpServlet 
{
    /**
	 * 
	 */
            ...
            throws ServletException, IOException 
    {
        ...
        System.out.println(" ");
        System.out.println("----------------------------------------");
        System.out.println("-----Debug Start Set processRequest-----");
        System.out.println("----------------------------------------");
        ...
        //if(AuthorizationServer.publicKey == null || AuthorizationServer.privateKey == null)
        if(this.getServletContext().getAttribute("publicKey") == null || this.getServletContext().getAttribute("privateKey") == null)
        {
        ...
        StringReader stringReader = new StringReader(requestParam);
        final JsonParser jsonParser = Json.createParser(stringReader);
        ...
        try
        {
            result = SetCommand(linkedHashMap);
    ...
    private String SetCommand(LinkedHashMap linkedHashMap) throws Exception 
    {
        String result = "";
            ...
            )
        {
            ...
            for (String key : keys) 
            {
                if (!key.equals("USP")) 
                {
                    //System.out.println("Row : " + rowCnt + " Value of " + key + " is: " + linkedHashMap.get(key).toString() );
                    if(key.endsWith("json"))
                    {
                        PGobject pGobject = new PGobject();
                        pGobject.setType("json");
                        pGobject.setValue(linkedHashMap.get(key).toString());
                        cstmt.setObject(rowCnt, pGobject);
                    }
                    else
                    {
...
*/
            ...
            if (inputStream != null) 
            {
                ...
                char[] charBuffer = new char[1024]; //4k, 사이즈 조정은 업무에 맞게
                int bytesRead = -1;
